...
**WRONG:** `memory_add(scope="read", ...)` → PermissionError: requires 'write' or higher.
**WRONG:** `memory_delete_all(scope="write", ...)` → PermissionError: requires 'admin'.
...
The error `Insufficient scope: 'all'. This tool requires 'read' or higher.` means the scope string was not recognized as a valid value. Always use one of the three valid strings.
